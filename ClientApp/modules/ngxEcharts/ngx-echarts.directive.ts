...
export class NgxEchartsDirective implements OnChanges, OnDestroy {
  ...
  private createChart() {
    ...
    let opts: {} = null;
    console.log(`CreateChart: width: ${(<Element>this.el.nativeElement).parentElement.offsetWidth} / height: ${(<Element>this.el.nativeElement).parentElement.offsetHeight}`);

    if ((<Element>this.el.nativeElement).parentElement.offsetWidth > 0 && (<Element>this.el.nativeElement).parentElement.offsetHeight > 0) {
      opts = {
        width: (<Element>this.el.nativeElement).parentElement.offsetWidth,
        height: (<Element>this.el.nativeElement).parentElement.offsetHeight
      };
    }

    if (this.theme) {
      return this._ngZone.runOutsideAngular(() => { return echarts.init(this.el.nativeElement, this.theme, opts) });
    } else {
      return this._ngZone.runOutsideAngular(() => { return echarts.init(this.el.nativeElement, null, opts) });
    }
  ...
  private hasData(): boolean {
    ...
    if (this.options.baseOption) {
      let baseOpt = this.options.baseOption;

      if (!baseOpt.series || !baseOpt.series.length) {
        return false;
      }

      for (let serie of baseOpt.series) {
        if (serie.data && serie.data.length > 0) {
          return true;
        }
      }
